...
class PixelDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        im = self.binary_loader(self.ims[index])
    ...
    def load_data(self):
        im = self.binary_loader(self.ims[self.index])
...
def pixel_loader(im_root, gt_root, batch_size, train_size, shuffle=True, num_worker=12, pin_memory=True):
    dataset = PixelDataset(im_root, gt_root, train_size)
...
def param_loader(im_root, gt_root, batch_size, train_size, shuffle=True, num_worker=12, pin_memory=True):
    dataset = ParamDataset(im_root, gt_root, train_size)
    data_loader = data.DataLoader(dataset=dataset,
                                  batch_size=batch_size,
                                  shuffle=shuffle,
                                  num_workers=num_worker,
...
class PointDataset(data.Dataset):
    def __init__(self, im_root, gt_root, resize, is_test=False):
        ...
        self.resize = resize
        self.ims = [im_root + f for f in os.listdir(im_root) if f.endswith('.jpg') or f.endswith('.png')]
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.csv')]
        self.ims = sorted(self.ims)
        self.gts = sorted(self.gts)
        self.size = len(self.ims)
...
def point_loader(im_root, gt_root, batch_size, train_size, shuffle=True, num_worker=12, pin_memory=True):
    dataset = PointDataset(im_root, gt_root, train_size)
    data_loader = data.DataLoader(dataset=dataset,
                                  batch_size=batch_size,
                                  shuffle=shuffle,
                                  num_workers=num_worker,
