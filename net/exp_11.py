...
class exp_11(nn.Module):
    def __init__(self, channel=64):
        super(exp_11, self).__init__()
        ...
        self.decoder_18 = self._make_layer(Bottle2neck, channel // 4, 3, stride=1)
        self.decoder_19 = self._make_layer(Bottle2neck, channel // 4, 3, stride=1)
        self.decoder_20 = self._make_layer(Bottle2neck, channel // 4, 3, stride=1)
        self.decoder_21 = self._make_layer(Bottle2neck, channel // 4, 3, stride=1)
        self.decoder_22 = self._make_layer(Bottle2neck, channel // 4, 3, stride=1)
        self.decoder_23 = self._make_layer(Bottle2neck, channel // 4, 3, stride=1)
        ...
        self.aspp_4 = ASPP(in_channel=channel, depth=channel)

        self.stage_1 = ConcatFusion(channel)
        self.stage_2 = ConcatFusion(channel)
        self.stage_3 = ConcatFusion(channel)
    ...
    def forward(self, x):
        ...
        x_s0, x_s1, x_s2, x_s3, x_s4, x_s5 = self.stage_3(x_s0, x_s1, x_s2, x_s3, x_s4, x_s5)

        x_s5 = self.aspp_4(x_s5)

        x_s0 = self.decoder_18(x_s0)
        x_s1 = self.decoder_19(x_s1)
        x_s2 = self.decoder_20(x_s2)
        x_s3 = self.decoder_21(x_s3)
        x_s4 = self.decoder_22(x_s4)
        x_s5 = self.decoder_23(x_s5)
