    ...
    for epoch in range(0, args.epoch):
        ...
        for step, data in enumerate(train_loader, start=1):
            for rate in size_rates:
                ...
                loss = loss_l2 + loss_sal + 0.1*loss_dice
                loss.backward()
            ...
            sw.add_scalars('loss', {'BCELoss': loss_sal_record.show(), 'MSELoss': loss_l2_record.show(),
                                    'DiceLoss': loss_dice_record.show()},
                           global_step=global_step)

            if step % 10 == 0 or step == total_step:
                print(
                    '{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], LR: {:.6f}, BCELoss: {:.4f}, MSELoss: {:.4f}, DiceLoss: {:.4f}'.
                        format(datetime.now(), epoch, args.epoch, step, total_step, scheduler.get_lr()[0],
                               loss_sal_record.show(), loss_l2_record.show(), loss_dice_record.show()), flush=True)
            global_step += 1
