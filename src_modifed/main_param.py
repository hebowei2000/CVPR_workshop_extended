    ...
    for epoch in range(0, args.epoch):
        ...
        for step, data in enumerate(train_loader, start=1):
            for rate in size_rates:
                ...
                loss_sal = nn.MSELoss()(pred_sal, gts)
                loss = loss_sal
                loss.backward()

                optimizer.step()
                log = 'Iteration: {:d} SalLoss: {:.4f}'.format(global_step,
                                                               loss_sal.data.cpu().numpy())
            ...
            sw.add_scalars('loss', {'SalLoss': loss_sal_record.show()},
                           global_step=global_step)

            if step % 10 == 0 or step == total_step:
                print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], LR: {:.6f}, SalLoss: {:.4f}'.
                      format(datetime.now(), epoch, args.epoch, step, total_step, scheduler.get_lr()[0],
                             loss_sal_record.show()), flush=True)
            global_step += 1
...
def main():
    ...
    # dataset
    args.data_path = '../data/param/'
    image_root = args.data_path + 'train/'
    gt_root = args.data_path + 'gt/'
    train_loader = param_loader(image_root, gt_root, args.batch_size, args.train_size)
    args.valset = ['test']
